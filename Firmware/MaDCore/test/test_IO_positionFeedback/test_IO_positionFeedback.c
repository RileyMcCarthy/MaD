...
{
    VIBES_TEST("encoder.init-starts-servo",
               "src/IO/IO_positionFeedback.c#IO_positionFeedback_init",
               "start-up of the servo position encoder");
    VIBES_EXPECT("servo-started", "the servo encoder hardware is started");
    VIBES_EXPECT("single-start", "exactly one encoder is started");
...
{
    VIBES_TEST("encoder.init-unknown-channel-is-noop",
               "src/IO/IO_positionFeedback.c#IO_positionFeedback_init",
               "start-up of a position encoder channel that does not exist");
    VIBES_EXPECT("nothing-started", "no encoder hardware is started");
...
void test_getValue_scales_steps_to_um(void)
{
    ...
    VIBES_EXPECT("position-in-um", "the position reads 2000 micrometres");
...
{
    VIBES_TEST("encoder.get-negative-steps",
               "src/IO/IO_positionFeedback.c#IO_positionFeedback_getValue",
...
{
    VIBES_TEST("encoder.zero-steps-per-mm-defaults-to-one",
               "src/IO/IO_positionFeedback.c#IO_positionFeedback_init",
...
{
    VIBES_TEST("encoder.get-unknown-channel-is-zero",
               "src/IO/IO_positionFeedback.c#IO_positionFeedback_getValue",
               "a position request on an encoder channel that does not exist");
    VIBES_EXPECT("reads-zero", "the position reads zero");
    IO_positionFeedback_init(IO_POSITION_FEEDBACK_CHANNEL_SERVO_FEEDBACK, 0, 200);
...
{
    VIBES_TEST("encoder.set-unknown-channel-refused",
               "src/IO/IO_positionFeedback.c#IO_positionFeedback_setValue",
