...
{
    VIBES_TEST("fw-gcode.missing-command-refused",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "no command text at all");
    VIBES_EXPECT("no-move", "no move is decoded");
...
{
    VIBES_TEST("fw-gcode.millimetres-become-micrometres",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a linear move of 10.5 millimetres at 2 millimetres per second, with a 100 millisecond pause");
    VIBES_EXPECT_WHY("millimetres-scaled",
                     "it is stored as 10500 micrometres of travel at 2000 micrometres per second",
                     "motion runs in micrometres; a millimetre left unscaled would be a thousand times too small");
    VIBES_EXPECT("pause-unscaled", "the pause is stored unchanged at 100 milliseconds");
    app_motion_move_t move;
...
{
    VIBES_TEST("fw-gcode.omitted-fields-are-zero",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a home command with no position, speed, or pause");
    VIBES_EXPECT("home-kept", "the home command is kept");
    VIBES_EXPECT("omitted-are-zero", "position, speed, and pause all read zero");
    app_motion_move_t move;
...
{
    VIBES_TEST("fw-gcode.leftover-fields-discarded",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a command that names only a new target position, after leftover command, position, speed, and pause from a previous line");
    VIBES_EXPECT("new-position-stored", "the new position is stored");
    VIBES_EXPECT_WHY("leftovers-cleared",
                     "the leftover command, speed, and pause are cleared to zero",
                     "leftover values from a previous line would send the gantry to the wrong place or at the wrong speed");
    app_motion_move_t move;
    move.g = 99;
    move.x = 123456;
    move.f = 654321;
...
{
    VIBES_TEST("fw-gcode.negative-position-keeps-sign",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a linear move to minus 2.5 millimetres");
    VIBES_EXPECT("sign-kept", "the target is stored as minus 2500 micrometres");
    app_motion_move_t move;
...
{
    VIBES_TEST("fw-gcode.micrometre-truncates-toward-zero",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a target position of 5.789 millimetres");
    VIBES_EXPECT("truncated-toward-zero",
                 "it is stored as 5789 micrometres, with any fraction of a micrometre dropped toward zero");
    app_motion_move_t move;
...
{
    VIBES_TEST("fw-gcode.unsupported-words-ignored",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a rapid move to 1 millimetre mixed with unsupported extra-axis words");
    VIBES_EXPECT("rest-of-line-stored", "the rapid command and its 1000 micrometre target are still stored");
    VIBES_EXPECT("extra-words-ignored", "the unsupported words land nowhere, leaving speed and pause at zero");
    app_motion_move_t move;
...
{
    VIBES_TEST("fw-gcode.multi-digit-stop-command",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a stop command whose number has more than one digit");
    VIBES_EXPECT("all-digits-read", "all of its digits are read and the command is stored as stop");
    app_motion_move_t move;
...
{
    VIBES_TEST("fw-gcode.repeated-position-last-wins",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a command that names two target positions");
    VIBES_EXPECT_WHY("last-position-stored",
                     "the last position named is the one stored",
...
{
    VIBES_TEST("fw-gcode.position-without-number-stays-zero",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a linear move that names a speed and a position with no number");
    VIBES_EXPECT("command-and-speed-kept", "the linear command and the speed are kept");
    VIBES_EXPECT("position-zero", "the position reads zero");
    app_motion_move_t move;
...
{
    VIBES_TEST("fw-gcode.empty-line-is-all-zero",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "an empty command line, after leftover command, position, speed, and pause");
    VIBES_EXPECT("line-accepted", "the line is accepted");
    VIBES_EXPECT("fields-cleared", "command, position, speed, and pause are all cleared to zero");
    app_motion_move_t move;
    move.g = 5;
    move.x = 5;
    move.f = 5;
...
{
    VIBES_TEST("fw-gcode.command-number-wraps-at-256",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "command numbers 256 and 257");
    VIBES_EXPECT_WHY("wrap-onto-rapid-and-linear",
                     "they are taken as a rapid move and a linear move respectively",
...
{
    VIBES_TEST("fw-gcode.pause-kept-in-milliseconds",
               "src/IO/IO_gcode.c#IO_gcode_decodeMove",
               "a pause of 5000 milliseconds");
    VIBES_EXPECT_WHY("milliseconds-kept",
                     "the pause is stored unchanged at 5000 milliseconds",
...
{
    VIBES_TEST("fw-gcode.end-test-command",
               "src/IO/IO_gcode.c#IO_gcode_isEndTest",
               "the end-of-test command");
    VIBES_EXPECT("end-of-test", "it is taken as the end of a test");
    TEST_ASSERT_TRUE(IO_gcode_isEndTest("G144"));
}

void test_isEndTest_null(void)
{
    VIBES_TEST("fw-gcode.missing-line-is-not-end-test",
               "src/IO/IO_gcode.c#IO_gcode_isEndTest",
               "a missing command line");
    VIBES_EXPECT("not-end-of-test", "it is not taken as the end of a test");
    TEST_ASSERT_FALSE(IO_gcode_isEndTest(NULL));
}

void test_isEndTest_nonMatching(void)
{
    VIBES_TEST("fw-gcode.nearby-commands-are-not-end-test",
               "src/IO/IO_gcode.c#IO_gcode_isEndTest",
               "a nearby command, a linear move, an empty line, and a lowercase end-of-test command");
    VIBES_EXPECT("none-match", "none of them is taken as the end of a test");
...
{
    VIBES_TEST("fw-gcode.end-test-requires-exact-line",
               "src/IO/IO_gcode.c#IO_gcode_isEndTest",
               "the end-of-test command with a leading space, a trailing space, or an extra digit");
    VIBES_EXPECT_WHY("none-match",
                     "none of them is taken as the end of a test",
