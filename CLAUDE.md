...
- **Vibes** (`Vibes/`) — the behaviour ledger that reports what each PR's tests claim. A **git submodule** → [RileyMcCarthy/vibes](https://github.com/RileyMcCarthy/vibes). MaD keeps its own `behaviours.jsonl`, `behaviours.next` and the `vibes.suite.json` files; tool changes land upstream first.
...
- **Submodules** (`SIL/embsim`, `Protocol/ProtoEmb`, `Vibes`): library changes land upstream ([embsim](https://github.com/RileyMcCarthy/embsim), [protoemb](https://github.com/RileyMcCarthy/protoemb), [vibes](https://github.com/RileyMcCarthy/vibes)) — commit + push there (their own CI gates them), then bump the pinned commit here in a MaD PR. Don't leave a MaD PR pointing at an unpushed submodule commit. `SIL/Cargo.toml` `exclude`s `embsim` (the submodule is its own Cargo workspace); its crates are consumed as path deps.
