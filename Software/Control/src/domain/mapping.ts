...
export function configToShared(
  ...
  return {
    ...
    // capacity is N in TS (codec applies the mN scale); sensitivity/zero are raw nV/V — display mV/V
    'Load Cell Capacity (N)': config.loadCellCapacity,
    'Load Cell Sensitivity (mV/V)': config.loadCellSensitivity / 1e6,
    'Load Cell Zero Balance (mV/V)': config.loadCellZeroBalance / 1e6,
...
export function configFromShared(
  ...
  return {
    ...
    loadCellCapacity: config['Load Cell Capacity (N)'],
    loadCellSensitivity: Math.round(config['Load Cell Sensitivity (mV/V)'] * 1e6),
    loadCellZeroBalance: Math.round(config['Load Cell Zero Balance (mV/V)'] * 1e6),
