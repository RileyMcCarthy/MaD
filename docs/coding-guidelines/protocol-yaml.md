...
- `Software/Control/src/protocol/generated/protoemb.ts` (deployed app) and `Software/MaDControl/src/main/generated/protoemb.ts` (Electron app)
...
- **TypeScript (shipped app):** the generated `protoemb.ts` must typecheck under the app's `tsc`/ESLint. After `npm run generate:proto`, run `npm run verify` in `Software/Control/` (tsc + eslint + Vitest + build). (Electron app: `npm run lint:fix` + `npm test` in `Software/MaDControl/`.)
