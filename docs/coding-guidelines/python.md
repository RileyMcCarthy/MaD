...
The Python's *output* — `Firmware/MaDCore/src/Generated/`, `Software/MaDWasmControl/src/protocol/generated/` (the shipped app; legacy Electron target was `Software/MaDControl/src/main/generated/`), and `Protocol/src/generated/` — is regenerated and **must not be hand-edited** (root `CLAUDE.md`; banner at `protocol.h.j2:2`). (Note: the SIL Rust output goes to `Protocol/src/generated/` per `SIL/Makefile:33`; the `SIL/embsim/peripherals/src/generated/` path some older docs reference does not exist.) To change generated code:
