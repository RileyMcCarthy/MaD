...
- [Vibes behaviour declarations (the test-derived ledger)](vibes-behaviours.md) — Annotating tests in TS/C/Rust so they publish claims to `behaviours.jsonl`: writing `given`/`then` for a reader who has never opened the repo (no internal type names or parameter letters), stable `id` semantics (reword freely, rename never), declare-on-entry and the C macro-placement rule, per-language binding snippets, and the `collect --write` + commit-the-ledger workflow.
...
- `Software/Control/src/protocol/generated/` (the deployed app)
...
| SIL (Rust) | `cargo clippy` + `cargo fmt --check` (from `SIL/`) | `make test` (emulator + `cargo test`) |
