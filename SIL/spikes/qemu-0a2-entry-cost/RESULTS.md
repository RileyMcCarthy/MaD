...
stock slice overhead is **2.279 ns/inst**, not 4.56. The 0c projection should be
read as a lower bound that was measured wrong, and this document replaces it.
...
| BQL around the slice | 205.5 | 6.2 | — |
...
```
  measured stock                           +3.209 ns/inst
    - locks/timers (REPLAY+LIMIT+BQL)      -0.684
    - Apple W^X toggles                    -0.379
    - nop-loop base (not slice overhead)   -0.930
  = irreducible slice overhead              1.216   (stock was 2.279)
```
...
|---|---|---|---|
| stock QEMU | 2.279 | 4.15 ns | ~14.5 |
| **after legitimate ablations** | **1.216** | **5.29 ns** | **~18.5** |
...
| BQL per slice | **yes** | QEMU already ships `stubs/iothread-lock.c`, a blessed no-op BQL — a link-time swap, not a hack |
