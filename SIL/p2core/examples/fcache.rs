...
fn main() {
    let path = std::env::args()
        .nth(1)
        .expect("usage: fcache <image> [skip]");
    let mut skip: u32 = std::env::args()
        .nth(2)
    ...
    println!(
        "at FCACHE jmp: PA(len)={pa}  PTRB(next)=${ptrb:05X}  src=${:05X}",
        ptrb - (pa + 1) * 4
    );
    ...
    for i in 0..(pa + 3).min(10) {
        ...
        let tag = if i == pa {
            "  <-- cog[PA], where ALTD put the _ret_ terminator"
        } else {
            ""
        };
