...
fn main() {
    ...
    let addr =
        u32::from_str_radix(a.next().expect("addr").trim_start_matches('$'), 16).expect("hex addr");
    ...
    println!(
        "value now {now:08X}, image had {orig:08X}{}",
        if now == orig { "" } else { "  <-- CHANGED" }
    );
    for h in m.watch_hits.iter().take(20) {
        println!(
            ...
            h.value,
            h.width,
            if h.addr != h.effective {
                "  <-- OUT OF RANGE, wrapped"
            } else {
                ""
            }
        );
